...
export function Sidebar({
  ...
  // Agrupación de chats por proyecto / workspace (como en Antigravity)
  const projectGroups = useMemo(() => {
    const map = new Map<
      string,
      { projectKey: string; projectName: string; projectPath: string; convos: Conversation[] }
    >();

    for (const convo of otherConversations) {
      const rawPath = convo.projectPath ? convo.projectPath.trim() : '';
      let projectKey = 'general';
      let projectName = 'General / Sin proyecto';
      let projectPath = '';

      if (rawPath) {
        projectPath = rawPath.replace(/\/+$/, '');
        projectKey = projectPath;
        if (projectPath === '/home/antonio' || projectPath.endsWith('/antonio')) {
      ...
      map.get(projectKey)!.convos.push(convo);
    }

    return Array.from(map.values()).sort((a, b) => {
      if (a.projectKey === 'general') return 1;
      if (b.projectKey === 'general') return -1;
      return a.projectName.localeCompare(b.projectName);
    });
  }, [otherConversations]);

  const toggleGroupCollapse = (projectKey: string) => {
  ...
  return (
    <aside className="w-64 h-full bg-surface border-r border-surface-border flex flex-col select-none shrink-0 relative">
      ...
      {/* Botón de Nuevo Chat y Sincronización */}
      <div className="p-3 flex items-center gap-2">
        <button
          type="button"
          onClick={onNewConversation}
          className="flex-1 flex items-center justify-center gap-2 py-2 px-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold shadow-md shadow-blue-600/20 transition-all active:scale-[0.98]"
        >
        ...
        {onSyncAntigravity && (
          <button
            ...
            disabled={isSyncing}
            className="p-2 rounded-xl bg-surface-elevated hover:bg-slate-700 text-slate-300 hover:text-white border border-surface-border text-xs transition-all shadow-sm shrink-0"
            title="Sincronizar chats de Antigravity (IDE y CLI)"
          >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-1 flex flex-col gap-0.5">
        {conversations.length === 0 ? (
          ...
          <>
            ...
            {/* Sección de Chats Agrupados por Proyecto / Workspace (Estilo Antigravity) */}
            {projectGroups.length > 0 && (
              <div className="flex flex-col gap-2.5">
                {projectGroups.map((group) => {
                  const isCollapsed = collapsedGroups.has(group.projectKey);
                  return (
                    <div key={group.projectKey} className="flex flex-col">
                      <button
                        type="button"
                        onClick={() => toggleGroupCollapse(group.projectKey)}
                        className="px-2 py-1.5 flex items-center justify-between text-[11px] font-semibold tracking-wide text-slate-400 hover:text-slate-200 transition-colors select-none rounded-lg hover:bg-slate-800/40 group/phead"
                        title={group.projectPath ? `Workspace: ${group.projectPath}` : 'Conversaciones sin workspace específico'}
                      >
                        <div className="flex items-center gap-1.5 truncate">
                          ...
                          <span className="truncate font-medium text-slate-300 group-hover/phead:text-white">
                            {group.projectName}
                          </span>
                        </div>
                      ...
                      {!isCollapsed && (
                        <div className="flex flex-col gap-0.5 mt-0.5 pl-1 border-l border-slate-800/60 ml-3">
                          {group.convos.map((convo) => renderConversationItem(convo))}
                        </div>
                      )}
                    </div>
                  );
